...
function validateProjection(p, scope){
 ...
 for(const f of projectionContract(scope).map(x=>x.name)){
  ...
  else if(f==='watermark') { if(!v||!Number.isSafeInteger(v.eventCount)||v.eventCount<0||!(v.latestCanonicalEventId===null||typeof v.latestCanonicalEventId==='string')||typeof v.canonicalDigest!=='string'||!v.canonicalDigest) return false; }
 }
 return typeof p.projectionContractVersion==='string'&&p.projectionContractVersion.length>0;
}
export function compareRebuiltVsPersistedProjection({expected, observed, checkedAt, reconciliationId, sourceContext}){
