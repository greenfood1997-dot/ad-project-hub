...
try {
  ...
  assert(!state.systemNotifications.some((item) => item.type === "project-assignment" && item.projectId === "p-target" && item.status === "待处理"), "保存分派后对应待分派提醒应自动处理");
  ...
  const created = await ok("POST", "/api/projects", "u-admin", {
    values: { "项目名称": "刚签约待分派项目", "客户 / 品牌": "新客户", "合同金额": 50000 },
    files: []
  });
  assert(created.project?.id, "管理员应能创建新项目");
  state = await ok("GET", "/api/state", "u-admin");
  assert(state.systemNotifications.some((item) => item.type === "project-assignment" && item.projectId === created.project.id && item.status === "待处理"), "新项目入库后应立即生成待分派提醒");
