...
async function call(method, path, userId, body) {
  ...
  try {
    ...
    payload = Buffer.concat(res.chunks).toString("utf8");
  }
...
const baseDb = {
  ...
  settings: {
    product: { "自动巡检间隔毫秒": "180000", "关闭自动巡检": "" },
  ...
  systemNotifications: [
    ...
    {
      id: "n-cash-runway",
      key: "company-cash-runway::company-cash-runway",
      type: "company-cash-runway",
      title: "公司现金流低于 6 个月安全线",
      text: "旧现金流待办",
      severity: "中",
      recipients: ["finance"],
...
try {
  ...
  const backupResult = await call("GET", "/api/admin/backup", "u-admin");
  assert(backupResult.status === 200, "管理员应能导出 OA 备份");
  const backup = typeof backupResult.payload === "string" ? JSON.parse(backupResult.payload) : backupResult.payload;
  assert(backup.format === "safe-backup-v1" && backup.counts.projects === 2, "备份应包含格式版本和项目数量");
  assert(backup.data.users.every((item) => !("pin" in item)), "备份不应包含成员 PIN");
  assert(backup.data.settings.feishu?.appSecret === "[已脱敏]", "备份应脱敏飞书密钥");
  const backupText = JSON.stringify(backup);
  assert(!backupText.includes("123456") && !backupText.includes("cli_mock_secret"), "备份原文不应泄露 PIN 或密钥");
  await denied("GET", "/api/admin/backup", "u-member", undefined, "普通员工不能导出全量 OA 备份");
  const validateBackupResult = await ok("POST", "/api/admin/backup/validate", "u-admin", { text: backupText });
  assert(validateBackupResult.ok === true && validateBackupResult.dryRunOnly === true, "管理员应能 dry-run 校验备份且不恢复");
  assert(validateBackupResult.counts.projects === 2 && validateBackupResult.currentCounts.projects === 2, "备份校验应返回备份数量和当前 OA 数量");
  assert(validateBackupResult.diff?.items?.some((item) => item.key === "projects" && item.current === 2 && item.backup === 2), "备份校验应返回恢复预演差异明细");
  await denied("POST", "/api/admin/backup/validate", "u-member", { text: backupText }, "普通员工不能校验全量 OA 备份");
  await denied("POST", "/api/admin/backup/validate", "u-admin", { text: "{bad json" }, "坏 JSON 应校验失败");
  const afterValidateState = await ok("GET", "/api/state", "u-admin");
  assert(afterValidateState.projects.length === 2 && afterValidateState.users.length === 6, "备份校验不能写入或覆盖现有数据");
  await denied("POST", "/api/admin/backup/restore", "u-member", { text: backupText, confirmText: "确认恢复OA备份" }, "普通员工不能恢复全量 OA 备份");
  await denied("POST", "/api/admin/backup/restore", "u-admin", { text: backupText, confirmText: "我随便点一下" }, "恢复备份必须输入确认语");
  const changedProject = await ok("POST", "/api/projects/update", "u-admin", {
    id: "p-visible",
    values: { "项目名称": "恢复前临时改名", "合同金额": 1 }
  });
  assert(changedProject.name === "恢复前临时改名", "恢复前应能先制造一个待恢复差异");
  const changedScheduler = await ok("POST", "/api/settings", "u-admin", {
    type: "product",
    values: { "自动巡检间隔毫秒": "300000", "关闭自动巡检": "" }
  });
  assert(changedScheduler.scheduler?.intervalMs === 300000, "恢复前应能先制造一个不同的巡检间隔");
  await ok("POST", "/api/projects/delete", "u-admin", { id: "p-hidden" });
  const changedValidateBackup = await ok("POST", "/api/admin/backup/validate", "u-admin", { text: backupText });
  assert(changedValidateBackup.diff?.changed?.some((item) => item.key === "projects" && item.delta === 1), "当前 OA 与备份不一致时校验应明确显示项目会恢复增加");
  const restoredBackup = await ok("POST", "/api/admin/backup/restore", "u-admin", { text: backupText, confirmText: "确认恢复OA备份" });
  assert(restoredBackup.restored === true && restoredBackup.counts.projects === 2, "管理员确认后应能恢复备份");
  assert(restoredBackup.diff?.changed?.some((item) => item.key === "projects" && item.delta === 1), "恢复结果应返回实际覆盖差异");
  assert(restoredBackup.scheduler?.enabled === true && restoredBackup.scheduler?.intervalMs === 180000, "恢复备份后后台巡检调度器应立即按备份设置重载");
  const restoredState = await ok("GET", "/api/state", "u-admin");
  assert(restoredState.projects.some((item) => item.id === "p-visible" && item.name === "员工可见项目" && item.contract === 100000), "恢复后项目应回到备份状态");
  assert(restoredState.settings.feishu?.appSecret === "cli_mock_secret", "恢复脱敏备份不应覆盖当前已有密钥");
  assert(restoredState.auditLogs.some((item) => item.type === "backup" && item.action === "restore"), "恢复备份应写入审计日志");
  assert(restoredState.auditLogs.some((item) => item.type === "backup" && item.meta?.diff?.changed?.some((diff) => diff.key === "projects" && diff.delta === 1)), "恢复审计应保留恢复预演差异");
  ...
  await denied("POST", "/api/company-finance", "u-member", { values: { currentCash: 999999 } }, "普通员工不应保存公司现金流");
  const financeSaved = await ok("POST", "/api/company-finance", "u-finance", {
    values: {
      currentCash: 300000,
      monthlyLaborCost: 60000,
      monthlyRent: 10000,
      monthlyLoan: 5000,
  ...
  assert(financeSaved.monthlyFixedCost === 80000 && financeSaved.safetyReserve === 480000, "公司现金流应按人力+租金+贷款+利息+其他计算 6 个月安全线");
  assert(/危险|现金偏紧/.test(financeSaved.runwayLabel), "公司现金流应返回安全线判断");
  const financeStateAfterSave = await ok("GET", "/api/state", "u-finance");
  assert(financeStateAfterSave.settings.companyFinance?.monthlyFixedCost === 80000, "财务保存现金流后应能从 state 读回结果");
  assert(financeStateAfterSave.systemNotifications.some((item) => item.id === "n-cash-runway" && item.status === "待处理" && /现金可撑/.test(item.text || "")), "现金流低于6个月时现金流待办应保持待处理并刷新文案");
  const financeSafe = await ok("POST", "/api/company-finance", "u-finance", {
    values: {
      currentCash: 1000000,
      monthlyLaborCost: 60000,
      monthlyRent: 10000,
  ...
  assert(financeSafe.runwayMonths >= 6 && financeSafe.runwayLabel === "现金安全线达标，可以稳健推进", "现金超过6个月安全线时应返回安全判断");
  const persistedFinance = JSON.parse(await readFile(dbFile, "utf8"));
  assert(persistedFinance.systemNotifications.some((item) => item.id === "n-cash-runway" && item.status === "已处理" && /超过 6 个月安全线/.test(item.note || "")), "现金流恢复安全后旧现金流待办应自动处理");
