...
const baseDb = {
  ...
  systemNotifications: [
    ...
    {
      id: "notice-verification-smoke-1",
      key: "verification-sheet-missing::p-smoke-1",
      type: "verification-sheet-missing",
      title: "本月核销表待上传",
      text: "捷途汽车测试项目本月还没有核销记录。",
      severity: "中",
      role: "pm",
      recipients: ["pm", "director"],
      projectId: "p-smoke-1",
...
try {
  ...
  assert(persisted.systemNotifications.some((item) => item.id === "notice-verification-smoke-1" && item.status === "已处理" && /核销表/.test(item.note || "")), "核销表确认入库后本月核销待办应自动处理");
  ...
  assert(Number(savedFinance.monthlyRent) === 10000, "公司现金流设置应保存");
  assert(savedFinance.monthlyFixedCost === 70000 && savedFinance.safetyReserve === 420000, "公司现金流设置应返回真实安全线计算");
