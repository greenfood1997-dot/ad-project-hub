...
const baseDb = {
  ...
  settings: {
    ...
    wechat: { webhookUrl: "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=mock", mockSend: "true" },
    members: {
      items: [
...
try {
  ...
  const reopenedVisibleNotice = await ok("POST", "/api/notifications/action", "u-member", {
    id: "n-visible",
    action: "reopen",
    note: "误点处理，恢复待办"
  });
  assert(reopenedVisibleNotice.status === "待处理", "成员应能恢复自己项目误处理的待办");
  assert(reopenedVisibleNotice.reopenedBy === "u-member" && reopenedVisibleNotice.reopenedAt, "待办恢复应保留恢复人和时间");
  ...
  await denied("POST", "/api/notifications/wechat/send", "u-member", {
    id: "n-visible"
  }, "普通成员不能发送企业微信待办通知");

  const wechatDelivery = await ok("POST", "/api/notifications/wechat/send", "u-pm", {
    id: "n-visible"
  });
  assert(wechatDelivery.mocked && wechatDelivery.sentBy === "u-pm", "PM 应能把自己可见项目待办发送到企业微信");

  const feishuDelivery = await ok("POST", "/api/notifications/feishu/send", "u-pm", {
    id: "n-visible"
  });
  assert(feishuDelivery.okCount >= 1 && feishuDelivery.missingCount >= 1, "飞书发送应返回成功和缺少绑定的收件人数量");
  assert(feishuDelivery.missingRecipients.some((item) => item.name === "执行成员"), "飞书发送应明确列出缺少 Open ID 的成员");

  const afterWechatState = await ok("GET", "/api/state", "u-pm");
  assert(afterWechatState.systemNotifications.some((item) => item.id === "n-visible" && item.wechatDelivery?.sentAt), "企业微信发送记录应写回系统待办");
  assert(afterWechatState.systemNotifications.some((item) => item.id === "n-visible" && item.feishuDelivery?.missingRecipients?.some((row) => row.name === "执行成员")), "部分飞书发送成功时缺少绑定名单应写回系统待办");

  const noBoundFeishu = await call("POST", "/api/notifications/feishu/send", "u-finance", {
    id: "n-company"
  });
  assert(noBoundFeishu.status === 400 && noBoundFeishu.payload.data?.missingRecipients?.some((item) => item.name === "财务"), "没有可发送对象时 API 应返回缺少飞书绑定名单");
  ...
  assert(adminState.auditLogs.some((item) => item.type === "notification" && item.action === "reopen" && item.target === "可见项目待办"), "待办恢复应进入审计日志");
