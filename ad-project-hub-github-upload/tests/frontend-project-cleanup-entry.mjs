...
assert(adminSource.includes("项目「${project.name}」已移入回收站"), "project cleanup should explain the completed recycle action");
assert(panelSource.includes("误建项目清理") && panelSource.includes("移入回收站"), "project cleanup should clearly move projects into the recycle bin");
...
assert(panelSource.includes("window.confirm") && panelSource.includes("30 天内可以恢复"), "project cleanup should keep one explicit recycle confirmation");
assert(panelSource.includes("关联记录") && panelSource.includes("云端原文件保留 30 天"), "project cleanup should explain scope and cloud retention before deletion");
