...
try {
  ...
  const outcome = await ok("POST", "/api/collections/outcome", "u-sales-new", {
    ...
    nextAction: "补发对账单后再次提醒客户财务"
  });
  assert(outcome.success === false && outcome.followUpStatus === "待跟进" && outcome.nextFollowUpAt === "2026-07-10", "待优化催收结果应可记录下次跟进计划");
  const followUpState = await ok("GET", "/api/state", "u-sales-new");
  assert(followUpState.systemNotifications.some((item) => item.type === "collection-follow-up" && item.sourceId === script.id && item.status === "待处理" && /补发对账单/.test(item.text || "")), "待优化催收应生成二次跟进待办");
  ...
  const closedFollowUpState = await ok("GET", "/api/state", "u-sales-new");
  assert(!closedFollowUpState.systemNotifications.some((item) => item.type === "collection-follow-up" && item.sourceId === script.id && item.status === "待处理"), "催收标记有效后二次跟进待办应关闭");
