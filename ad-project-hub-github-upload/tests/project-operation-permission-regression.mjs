...
const baseDb = {
  ...
  systemNotifications: [
    { id: "notice-visible", type: "project-progress-lag", projectId: "p-visible", projectName: "可操作项目", status: "待处理", title: "可见项目进度提醒" },
    { id: "notice-visible-cost", type: "project-cost-pressure", projectId: "p-visible", projectName: "可操作项目", status: "待处理", title: "可见项目成本提醒" },
    { id: "notice-visible-verification", type: "verification-sheet-missing", projectId: "p-visible", projectName: "可操作项目", status: "待处理", title: "可见项目核销提醒" },
    { id: "notice-visible-supplier", type: "supplier-settlement-pending", projectId: "p-visible", projectName: "可操作项目", status: "待处理", title: "可见项目供应商提醒" },
    { id: "notice-visible-receivable", type: "project-receivable-risk", projectId: "p-visible", projectName: "可操作项目", status: "待处理", title: "可见项目回款提醒" },
...
try {
  ...
  const closed = await ok("POST", "/api/projects/update", "u-pm", {
    id: "p-visible",
    values: {
      "项目名称": "可操作项目",
      "项目状态": "已完成",
      "结案时间": "2026-07-04T08:00:00.000Z",
      "结案复盘备注": "最大支出来自达人拍摄，下次提前锁价。"
    }
  });
  assert(closed.status === "已完成", "PM 应能把自己项目标记为已完成");
  assert(closed.closedAt === "2026-07-04T08:00:00.000Z", "结案时间应写入项目");
  assert(closed.closeoutNote.includes("提前锁价"), "结案复盘备注应写入项目");
  let persistedAfterCloseout = JSON.parse(await readFile(dbFile, "utf8"));
  const closeoutExecutionNotices = persistedAfterCloseout.systemNotifications.filter((item) => ["notice-visible", "notice-visible-cost", "notice-visible-verification", "notice-visible-supplier"].includes(item.id));
  assert(closeoutExecutionNotices.length === 4 && closeoutExecutionNotices.every((item) => item.status === "已处理"), "项目结案后执行类待办应自动处理");
  assert(persistedAfterCloseout.systemNotifications.some((item) => item.id === "notice-visible-receivable" && item.status === "待处理"), "项目结案后回款待办应继续保留");
