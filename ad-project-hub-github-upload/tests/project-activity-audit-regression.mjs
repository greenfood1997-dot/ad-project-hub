...
try {
  ...
  assert(comment.id && comment.userId === "u-member", "项目动态应保留稳定 ID 和记录人 ID");
  ...
  const archivedComment = await ok("POST", "/api/comments/archive", "u-member", {
    project: "动态测试项目",
    commentId: comment.id,
    reason: "记录错了，归档重写"
  });
  assert(archivedComment.archivedAt && archivedComment.archivedBy === "u-member", "记录人应能归档自己的项目动态");
  assert(archivedComment.archiveReason === "记录错了，归档重写", "动态归档原因应被保存");

  const pmComment = await ok("POST", "/api/comments", "u-member", {
    project: "动态测试项目",
    body: "这条由 PM 归档"
  });
  const pmArchived = await ok("POST", "/api/comments/archive", "u-pm", {
    projectId: "p-activity",
    commentId: pmComment.id,
    reason: "PM 清理错误动态"
  });
  assert(pmArchived.archivedBy === "u-pm", "项目管理角色应能归档可见项目动态");
  ...
  assert(memberState.comments.some((item) => item.project === "动态测试项目" && item.body.includes("客户已确认") && item.archivedAt), "成员应能看到自己项目已归档评论用于审计");
  ...
  assert(memberState.auditLogs.some((item) => item.type === "comment" && item.action === "archive" && item.target === "动态测试项目"), "成员应看到自己项目动态归档审计日志");
  ...
  await denied("POST", "/api/comments/archive", "u-outsider", {
    project: "动态测试项目",
    commentId: comment.id
  }, "无关成员不应归档不可见项目动态");
